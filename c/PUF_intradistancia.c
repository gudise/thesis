...
int main(int N_opcion, char **opcion)
{
    ...
    PUFEXP* experimento;
    MATRIZ* set_intradist;
    DISTRI* hist_intradist, *ajuste_binomial;
    ...
    for(i=0; i<N_opcion; i++)
    {
        ...
        {
            printf("\nintradistancia:\n----------------------------------\nEste programa calcula la distribucion de intradistancias en el experimento 'input.pex', y devuelve el histograma normalizado de dicha distribucion asi como su ajuste a una binomial. El nombre del ajuste incluira el valor de los parametros de la binomial 'p' y 'n' (= N_bits).--------------------------------------------------------:\n\n");
            printf("-in [input.pex], nombre del fichero de entrada (formato putty)\n\n");
            printf("-out [output.txt], nombre del fichero de salida (histograma de 0 a 100).\n\n");
            ...
            printf("-caja [1.0], tamano de la caja para el histograma. Por defecto se utilizan tantas cajas como items se leen de 'input.pex', i.e., bin=1.0\n\n");
    ...
    set_intradist = intradistanciaSet(experimento, realdata);

    hist_intradist = construyeHistograma(set_intradist, caja, -1, -1, normalizacion, 1, 0);
    pintarDistribucion(hist_intradist, output);
    
    media=0;
    for(i=0; i<set_intradist->N_filas; i++)
        media+=set_intradist->elemento[i][0];
    media/=set_intradist->N_filas;
    
    desviacion = 0;
    for(i=0; i<set_intradist->N_filas; i++)
        desviacion+=(set_intradist->elemento[i][0]-media)*(set_intradist->elemento[i][0]-media);
    desviacion/=(set_intradist->N_filas-1);
    
    if(verbose)
		printf("\nIntra-distancia media = %lg ( %lg %% )\nDesviacion estandar = %lg ( %lg %% )\n", media, media/experimento->tamano_respuesta*100, sqrt(desviacion/set_intradist->N_filas),sqrt(desviacion/set_intradist->N_filas)/experimento->tamano_respuesta*100);
	else if(!ajuste)
		printf("%lg", media/experimento->tamano_respuesta*100);
    
	if(ajuste)
    {
        ajuste_binomial = ajustarBinomial(hist_intradist, experimento->tamano_respuesta, N_iteraciones);
    ...
    freeMatriz(set_intradist);
    freeDistribucion(hist_intradist);
