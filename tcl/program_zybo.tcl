...
jtag targets -filter {jtag_cable_name =~ "Digilent Zybo 210279A78FA0A" && level==1}

targets -set -filter {jtag_cable_name =~ "Digilent Zybo 210279A78FA0A" && level==0} -index 1
...
jtag targets -set -filter {name =~ "Digilent Zybo 210279A78FA0A" && level == 0}

jtag frequency

source ${sdkdir}/design_1_wrapper_hw_platform_0/ps7_init.tcl

targets -set -nocase -filter {name =~"APU*" && jtag_cable_name =~ "Digilent Zybo 210279A78FA0A"} -index 0
...
targets -set -nocase -filter {name =~ "ARM*#0" && jtag_cable_name =~ "Digilent Zybo 210279A78FA0A"} -index 0

rst -processor

targets -set -nocase -filter {name =~ "ARM*#0" && jtag_cable_name =~ "Digilent Zybo 210279A78FA0A"} -index 0

dow ${sdkdir}/${app_name}/Debug/${app_name}.elf
