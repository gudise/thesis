...
do
	...
	then
		echo "
Este template genera los archivos necesarios para crear un sistema de comunicación Arduino->FPGA. El sistema consta de 3 cables entre el Arduino y la FPGA: data, avail y stored. El funcionamiento es muy simple: el Arduino coloca un bit en data, y sube avail->HIGH. Entonces el módulo de la FPGA leerá el bit, y subirá stored->HIGH. Cuando el arduino ve stored en HIGH, baja avail->LOW. Cuando el módulo de la FPGA ve avail en LOW, vuelve al estado de inicio. Este proceso se repite BUFFER_OUT_WIDTH veces; una vez termina, el módulo de la FPGA va al estado DONE y sube el pin de salida done->HIGH. Cuando el módulo de la FPGA lee reset en HIGH, se resetea y vuelve al estado inicial para leer otros BUFFER_OUT_WIDTH bits. Asegurarse de que el número BUFFER_OUT_WIDTH es el mismo en el programa del arduino y en el módulo de la FPGA! El código de ejemplo del Arduino simplemente envía 1000 números (0, 999) secuencialmente a la FPGA. Del lado de la FPGA se genera un block_design y un programa .c para SDK que permite visualizar el ejemplo. Ojo! Es importante preparar el arduino (encenderlo y abrir el monitor serie) antes de encender la FPGA! (si no da problemas).
--------------
	
	Opciones:
	
		-help, pinta esta ayuda.
		-tx, si esta opción está presente el script devuelve alternativamente los archivos necesarios para un sistema de comunicación FPGA->Arduino. Notar que los sufijos 'rx' y 'tx' ahora están cambiados.
		"
			exit
    elif test $opcion = "-tx"
    then
...
if test $TX -eq 0
then
	cp "$COMPUTACION/my-source-repo/verilog/async-comm-rx.v" ./vivado-src/
else
	cp "$COMPUTACION/my-source-repo/verilog/async-comm-tx.v" ./vivado-src/
fi
...
#set_property -dict { PACKAGE_PIN M15   IOSTANDARD LVCMOS33 } [get_ports { done }]; #IO_L23N_T3_35 Sch=led5_r
...
set_property -dict { PACKAGE_PIN U13   IOSTANDARD LVCMOS33 } [get_ports { data }]; #IO_L3P_T0_DQS_PUDC_B_34 Sch=ar[2]
set_property -dict { PACKAGE_PIN V13   IOSTANDARD LVCMOS33 } [get_ports { available }]; #IO_L3N_T0_DQS_34 Sch=ar[3]
set_property -dict { PACKAGE_PIN V15   IOSTANDARD LVCMOS33 } [get_ports { stored }]; #IO_L10P_T1_34 Sch=ar[4]
...
" > ./vivado-src/async-comm.xdc

mkdir arduino
if test $TX -eq 0
then
	cp "$COMPUTACION/my-source-repo/c-arduino/async-comm-tx.ino" ./arduino/
else
	cp "$COMPUTACION/my-source-repo/c-arduino/async-comm-rx.ino" ./arduino/
fi

mkdir block-design
if test $TX -eq 0
then
	cp "$COMPUTACION/my-source-repo/tcl/bd_async-comm-rx_zynq.tcl" ./block-design/
else
	cp "$COMPUTACION/my-source-repo/tcl/bd_async-comm-tx_zynq.tcl" ./block-design/
fi

mkdir sdk-src
if test $TX -eq 0
then
	cp "$COMPUTACION/my-source-repo/c-xilinx/async-comm-rx.c" ./sdk-src/
else
	cp "$COMPUTACION/my-source-repo/c-xilinx/async-comm-tx.c" ./sdk-src/
fi
