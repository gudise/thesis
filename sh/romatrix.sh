...
VIVADO_FILES="${PROJDIR}/vivado_src/top.v ${PROJDIR}/vivado_src/interfaz_pspl.cp.v ${PROJDIR}/vivado_src/romatrix.v ${PROJDIR}/vivado_src/medidor_frec.cp.v"
if test "$PBLOCK" != "no"
then
...
cp "$REPO_fpga/verilog/medidor_frec.v" ./vivado_src/medidor_frec.cp.v

gen_romatrix.py -out "romatrix" -Ninv $NINV -Nosc $NOSC -posmap $POSMAP -tipo $TIPO -pinmap $PINMAP -minsel $MINSEL -resolucion $RESOLUCION $DEBUG
