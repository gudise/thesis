...
if BUFFER_IN_WIDTH==0:
	BUFFER_IN_WIDTH = NBITSOSC+NBITSPDL+NBITSPOLY+5+5
...
genGaromatrix(out_name="vivado_src/garomatrix.v", N_inv=NINV, tipo=TIPO, pinmap_opt=PINMAP, minsel=MINSEL, posmap=POSMAP)
...
with open("vivado_src/top.v", "w") as f:
	...
	if TIPO == "lut3":
		...
		aux1=f".sel_pdl(buffer_in[{NBITSOSC+NBITSPDL-1}:{NBITSOSC}]),"
...
module TOP (
	...
	GAROMATRIX garomatrix (
		...
		.enable(1'b1),
		.clock_s(clock_s),
		.sel_poly(buffer_in[{NBITSOSC+NBITSPDL+NBITSPOLY-1}:{NBITSOSC+NBITSPDL}]),
		{aux}
		{aux1}
		.out(out_ro)
	);
	...
	MEDIDOR_BIAS #(
		...
		.muestra(out_ro),
		.resol(buffer_in[{NBITSOSC+NBITSPDL+NBITSPOLY+5-1}:{NBITSOSC+NBITSPDL+NBITSPOLY}]),
		.lock(ack),
		.out(buffer_out)
	);
...
print(f"""
 ...
 Trama del selector(ts): {NBITSOSC} {NBITSPDL} {NBITSPOLY} 5 5
 sel_ro_width = {NBITSOSC}
 sel_pdl_width = {NBITSPDL}
 sel_poly_width = {NBITSPOLY}
 dw  = {DATA_WIDTH}
 biw = {BUFFER_IN_WIDTH}
