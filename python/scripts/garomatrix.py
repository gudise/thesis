...
for i, opt in enumerate(argv): # "i" es el indice que recorre la lista de opciones, "opt" es la opcion propiamente dicha
	if opt == "-help":
		with open(f"{os.environ['REPO_fpga']}/python/scripts/help/garomatrix.help", "r") as f:
			print(f.read())
		exit()
...
vivado_files=f"{proj_dir}/vivado_src/top.v {proj_dir}/vivado_src/interfaz_pspl.cp.v {proj_dir}/vivado_src/garomatrix.v {proj_dir}/vivado_src/medidor_bias.cp.v {proj_dir}/vivado_src/clock_divider.cp.v {proj_dir}/vivado_src/interfaz_pspl_config.vh"
if pblock != "no":
...
run(["cp",f"{os.environ['REPO_fpga']}/verilog/medidor_bias.v","./vivado_src/medidor_bias.cp.v"])
...
genGaromatrix(out_name="vivado_src/garomatrix.v", N_inv=N_inv, tipo=tipo_lut, pinmap_opt=pinmap, minsel=minsel, posmap=posmap)
...
with open("vivado_src/top.v", "w") as f:
	...
	if tipo_lut == "lut3":
		...
		aux1=f".sel_pdl(buffer_in[{N_bits_osc+N_bits_pdl-1}:{N_bits_osc}]),"
...
module TOP (
	...
	CLOCK_DIVIDER clock_divider (
		.clock_in(clock),
		.fdiv(buffer_in[{buffer_in_width-1}:{buffer_in_width-5}]),
		.clock_out(clock_s)
	);
	...
	GAROMATRIX garomatrix (
		...
		.enable(1'b1),
		.clock_s(clock_s),
		.sel_poly(buffer_in[{N_bits_osc+N_bits_pdl+N_bits_poly-1}:{N_bits_osc+N_bits_pdl}]),
		{aux}
		{aux1}
		.out(out_ro)
	);
	...
	MEDIDOR_BIAS #(
		...
		.muestra(out_ro),
		.resol(buffer_in[{N_bits_osc+N_bits_pdl+N_bits_poly+5-1}:{N_bits_osc+N_bits_pdl+N_bits_poly}]),
		.lock(ack),
		.out(buffer_out)
	);
...
print(f"""
 ...
 Trama del selector(ts): {N_bits_osc} {N_bits_pdl} {N_bits_poly} 5 5
 
	(0) <---sel_ro({N_bits_osc})---><---sel_pdl({N_bits_pdl})---><---sel_poly({N_bits_poly})---><---sel_resol(5)---><---sel_fdiv(5)---> ({buffer_in_width-1})
